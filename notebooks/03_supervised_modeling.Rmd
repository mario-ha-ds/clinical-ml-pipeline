...
# Load the 10-fold cross-validation objects built in Notebook 01
cancer_folds <- base::readRDS("../data/split/cancer_folds.rds")
cvd_folds    <- base::readRDS("../data/split/cvd_folds.rds")

# Load the unprepped base recipes for the _logistic family
...
# --- A. Breast Cancer: binary specification ---
# Both hyperparameters (penalty and mixture) are marked for tuning
...
# --- B. CVD: multinomial specification ---
# Same tunable hyperparameters, using the multinomial engine for the three-class target
...
# --- A. Breast Cancer workflow ---
# Bundle the recipe and the binary specification into a single object
cancer_log_wf <- workflows::workflow() |>
  workflows::add_recipe(cancer_rec_logistic) |>
  workflows::add_model(log_spec_bin)

# --- B. CVD workflow ---
# Bundle the recipe and the multinomial specification into a single object
...
# Build a regular 5x5 grid over penalty (log scale) and mixture
...
# --- A. Grid search: Breast Cancer ---
# Evaluate every combination across the 10 folds, using the clinical metric panel
base::set.seed(42)
...
# --- B. Grid search: CVD ---
# Same procedure on the cardiovascular folds
base::set.seed(42)
...
# Extract the winning hyperparameter combination under the clinical hierarchy
best_cancer_log_params <- get_best_hierarchical(cancer_log_tune)
best_cvd_log_params    <- get_best_hierarchical(cvd_log_tune)

# Isolate the hyperparameter values for final model fitting
...
# Display the winning configuration: Breast Cancer
...
# Display the winning configuration: CVD
base::print(knitr::kable(
  best_cvd_log_params,
  format = "simple",
  digits = 3,
...
```{r import-train-data}
# Load the raw (unprepped) training splits, needed to fit the final workflows
...
# --- A. Fit the final Breast Cancer model ---
# Inject the winning hyperparameters into the workflow
final_cancer_log_wf <- cancer_log_wf |>
  tune::finalize_workflow(best_cancer_log)

# Fit on the full raw training set; the recipe steps are applied internally by the workflow
base::set.seed(42)
cancer_log_fit <- final_cancer_log_wf |>
  parsnip::fit(data = cancer_train_raw) # Note: uses the raw data here!

# --- B. Fit the final CVD model ---
# Inject the winning hyperparameters into the workflow
final_cvd_log_wf <- cvd_log_wf |>
  tune::finalize_workflow(best_cvd_log)

# Fit on the full raw training set
base::set.seed(42)
...
# Extract every coefficient, sorted by absolute magnitude: Breast Cancer
...
# Extract every coefficient, grouped by class and sorted by absolute magnitude: CVD
...
# Prep the recipes explicitly to reuse the same baked data in later sections,
# instead of pulling the transformation directly from the trained workflow
...
# Bake the training set through each recipe
...
# --- B. Model extraction ---
# Pull the fitted parsnip model out of each workflow
cancer_log_model <- workflows::extract_fit_parsnip(cancer_log_fit)
cvd_log_model    <- workflows::extract_fit_parsnip(cvd_log_fit)

# --- C. Probability prediction and target linking ---
# Predict class probabilities and bind the true target column back on
...
# --- D. Hard-class assignment ---
# Breast Cancer scenario (binary): a 50% threshold assigns the malignant class
...
# Preview the resulting predictions: Breast Cancer
utils::head(cancer_log_train_preds)
```

```{r internal-eval-preds-logistic-head-cvd}
# Preview the resulting predictions: CVD
...
# Re-align the factor levels defensively before scoring
cancer_log_train_preds <- cancer_log_train_preds |>
  dplyr::mutate(.pred_class = base::factor(.pred_class, levels = base::levels(diagnosis)))

# Compute the full clinical metric panel, treating the second factor level (Malignant) as the event of interest
...
# Build and display the confusion matrix: Breast Cancer
...
# Lower the decision threshold to 30% to test whether Sensitivity improves at the cost of Specificity
...
# Recompute the metrics and confusion matrix under this adjusted threshold
...
# Display the metrics at the adjusted threshold
...
# Display the confusion matrix at the adjusted threshold
...
# Compute the full clinical metric panel across the three CVD risk classes
...
# Build and display the confusion matrix: CVD
...
# Serialize both final fitted workflows to disk
...
# Load the unprepped base recipes (biological units preserved)
...
# --- A. Breast Cancer cost matrix ---
# A missed malignancy (false negative) is penalized five times more heavily
...
# --- B. CVD cost matrix ---
# Costs scale with clinical distance: confusing HIGH for LOW is penalized far
# more heavily than confusing HIGH for INTERMEDIARY
...
```{r define-c50-spec}
# --- A. Breast Cancer: specification with its cost matrix ---
...
# --- B. CVD: specification with its cost matrix ---
...
# --- A. Breast Cancer workflow ---
# Bundle the _trees recipe (biological units, no scaling) with the C5.0 specification
...
```{r tune-c50-grid}
# --- A. Grid definition ---
# Explore min_n over a wider, higher range for CVD, given its larger sample size and noisier signal
...
# --- B. Grid search: Breast Cancer ---
# Evaluate every min_n value across the 10 folds, using the hard-metric panel
# (no AUC, since the cost matrix distorts the native probabilities)
base::set.seed(42)
...
```{r fit-final-c50}
# --- A. Fit the final Breast Cancer tree ---
...
# --- A. Recipe preparation ---
# Prep and bake the _trees recipes on the raw training set
...
# --- C. Hard-class prediction and target linking ---
# C5.0 with a cost matrix only exposes hard classes, so type = "class" is used directly
...
# Display the internal metrics: Breast Cancer
...
# Display the internal metrics: CVD
...
```{r define-rf-spec}
# --- A. Breast Cancer: specification with a fixed forest size ---
...
# --- B. CVD: specification with the same fixed forest size ---
...
# --- A. Breast Cancer workflow ---
# Reuse the same _trees recipe as C5.0, since Random Forest is also scale-invariant
...
```{r tune-rf-grid}
# --- A. Grid definition ---
# mtry range is capped by each dataset's actual predictor count
...
# Extract the best combination under the integral metric hierarchy
best_cancer_rf_params <- get_best_hierarchical(cancer_rf_tune)
best_cvd_rf_params    <- get_best_hierarchical(cvd_rf_tune)

# Isolate the hyperparameters needed for final training
...
# --- A. Fit the final Breast Cancer forest ---
...
# --- C. Probability and hard-class prediction, and target linking ---
# Unlike C5.0, Random Forest exposes clean probabilities, so both "prob" and "class" are pulled
...
# --- E. Clinical metric evaluation ---
# AUC is meaningful here, since Random Forest carries no cost matrix distorting the probabilities
