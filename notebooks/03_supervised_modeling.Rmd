...
# --- A. Cross-validation folds (Notebook 01) ---
# Import the rset objects, one 10-fold structure per dataset
cancer_folds <- base::readRDS("../data/split/cancer_folds.rds")
cvd_folds    <- base::readRDS("../data/split/cvd_folds.rds")

# --- B. Unprepped recipes (Notebook 01) ---
# Import the "_logistic" recipes; they are not yet trained on any data, so
# each cross-validation fold can prep() and bake() them independently
...
# --- A. Binary specification (Breast Cancer) ---
...
# --- B. Multinomial specification (CVD, 3 classes) ---
...
# --- A. Workflow for Breast Cancer ---
# Bundle the binary recipe together with the binary model specification
cancer_log_wf <- workflows::workflow() |>
  workflows::add_recipe(cancer_rec_logistic) |>
  workflows::add_model(log_spec_bin)

# --- B. Workflow for Cardiovascular Risk ---
# Same idea, using the multinomial recipe and the multinomial specification
...
# Define the hyperparameter search grid: 5 levels of penalty x 5 of mixture
...
# --- A. Cross-validation for Breast Cancer ---
base::set.seed(42)
...
# --- B. Cross-validation for Cardiovascular Disease ---
# Same grid and metric set, run on the CVD folds and workflow
base::set.seed(42)
...
# --- A. Extract the winning hyperparameters under the clinical hierarchy ---
best_cancer_log_params <- get_best_hierarchical(cancer_log_tune)
best_cvd_log_params    <- get_best_hierarchical(cvd_log_tune)

# --- B. Isolate just the hyperparameter values, needed for final fitting ---
...
```{r import-train-data}
# --- A. Breast Cancer: raw train set ---
cancer_train_raw <- base::readRDS("../data/split/cancer_train_base.rds")

# --- B. Cardiovascular Disease: raw train set ---
...
# --- A. Finalize and train the final model: Breast Cancer ---
...
# --- B. Finalize and train the final model: CVD Risk ---
...
# --- A. Breast Cancer: extract all coefficients, sorted by absolute weight ---
...
# --- B. CVD (multinomial): same extraction, sorted within each class ---
...
# This could be pulled directly from the trained workflow, but it is done explicitly here for clarity
...
# Apply the transformations (bake), using new_data = *_train_raw to process the entire
...
# Convert the result into a factor with the same levels as the original target, to avoid
...
# Dynamically copy the levels from the original target into the predictions.
cancer_log_train_preds <- cancer_log_train_preds |>
  dplyr::mutate(.pred_class = base::factor(.pred_class, levels = base::levels(diagnosis)))

# --- Breast Cancer evaluation (binary) ---
# Pass the probability of the pathological class (Malignant)
...
# Generate the confusion matrix using the centralized plotting function
...
# Adjust the cut-off point manually to prioritize capturing true positives (Sensitivity)
...
# Evaluate the impact using the centralized evaluation utils
...
# Ensure the predicted class factor uses the same levels as the true target
cvd_log_train_preds <- cvd_log_train_preds |>
  dplyr::mutate(.pred_class = base::factor(.pred_class, levels = base::levels(cvd_risk_level)))

# --- CVD evaluation (multinomial) ---
# Being multiclass, the probabilities of all three classes are passed simultaneously
...
# Generate the confusion matrix for the Cardiovascular Risk scenario
...
# --- A. Extract the native engine and inspect the tree/rules: Breast Cancer ---
...
# --- B. Extract the native engine and inspect the tree/rules: CVD Risk ---
...
# Dynamically synchronize the original target levels into the predictions
...
# --- A. Confusion matrix: Breast Cancer ---
...
rf_grid_cvd <- dials::grid_regular(
  # CVD has fewer variables (15), so the maximum is capped at 14
...
# --- A. Variable importance extraction (Gini): Breast Cancer (26 variables) ---
...
# --- B. Variable importance extraction (Gini): Cardiovascular Risk (15 variables) ---
...
# Unlike C5.0, here both continuous PROBABILITIES and the hard CLASS are extracted.
